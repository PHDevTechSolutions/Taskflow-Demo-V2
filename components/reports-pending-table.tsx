...
export const PendingTable: React.FC<NewTaskProps> = ({
    ...
    const [history, setHistory] = useState<HistoryRecord[]>([]); // <-- New state for history
    const [loadingCompanies, setLoadingCompanies] = useState(false);
    const [loadingActivities, setLoadingActivities] = useState(false);
    const [loadingHistory, setLoadingHistory] = useState(false); // <-- New loading state for history
    const [errorCompanies, setErrorCompanies] = useState<string | null>(null);
    const [errorActivities, setErrorActivities] = useState<string | null>(null);
    const [errorHistory, setErrorHistory] = useState<string | null>(null); // <-- New error state for history
    ...
    const fetchHistory = useCallback(async () => {
        ...
        try {
            const res = await fetch(`/api/act-fetch-history?referenceid=${encodeURIComponent(referenceid)}`, {
                cache: "no-store",
                headers: {
                    "Cache-Control": "no-store, no-cache, must-revalidate, proxy-revalidate",
                    Pragma: "no-cache",
            ...
            if (!res.ok) {
                const json = await res.json();
                throw new Error(json.error || "Failed to fetch history data");
            }
    ...
    // useEffect to call fetchActivities and fetchHistory on mount / ref change
    useEffect(() => {
        fetchActivities();
        fetchHistory();
    ...
    const mergedData = useMemo(() => {
        // Map for quick lookup of history by activity_reference_number
        const historyMap = new Map(
            history.map((h) => [h.activity_reference_number, h])
        );
        ...
        return activities
            ...
            .map((activity) => {
                ...
                const historyRecord = historyMap.get(activity.activity_reference_number);
                ...
                return {
                    ...
                    so_number: historyRecord?.so_number ?? "-",
                    so_amount: historyRecord?.so_amount ?? "-",
                };
            })
            .sort(
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            {/* New Columns for SO Number and SO Amount */}
