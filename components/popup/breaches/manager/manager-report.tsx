...
export default function TSMReports() {
  ...
  const fetchOverdue = useCallback(async (refId: string, from: string, to: string) => {
    ...
    try {
      const url = `/api/activity/manager/breaches/fetch?manager=${encodeURIComponent(refId)}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const agentCount = totalAgents > 0 ? totalAgents : (fixedCount || 1);
      const dailyDenom = agentCount * 20;
      const weeklyDenom = dailyDenom * 6;
      const monthlyDenom = dailyDenom * workingDays; // ← was * 26
