...
export default function TSAReports() {
    ...
    // Currently selected agent to view
    ...
    const [denominators, setDenominators] = useState<Denominators>({
        ...
        csrClient: 0, newClient: 0, tsaClient: 0,
        daily: 20, weekly: 100, monthly: 440,
    });
    ...
    useEffect(() => {
        ...
        const fetchAgents = async () => {
            ...
            try {
                const res = await fetch(`/api/activity/tsm/breaches/fetch-agent?id=${encodeURIComponent(managerDetails.referenceid)}`);
                if (!res.ok) throw new Error();
                const data: Agent[] = await res.json();
                const active = data.filter((a) => (a.Status || "").toLowerCase() === "active");
                setAgents(active);
                // Auto-select first agent
                if (active.length > 0 && !selectedRefId) {
                    setSelectedRefId(active[0].ReferenceID);
                }
            } catch {
        ...
        fetchAgents();
        // Only fetch when manager changes, not when selectedRefId changes
    ...
    // ─── Auto-fetch whenever selectedRefId or date changes ─────────────────

    useEffect(() => {
        if (!selectedRefId) return;
        // Reset metrics when switching agents
        setActivities([]);
        setOverdueByCompany({});
        setOverdueCount(0);
    ...
    // ─── Compute daily/weekly/monthly outbound + time metrics ──────────────
