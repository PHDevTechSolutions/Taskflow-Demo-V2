...
export function AccountsTable({ posts, userDetails, dateCreatedFilterRange }: AccountsTableProps) {
  ...
  const hasAgentFilter = ownerFilter !== "all";
  ...
  useEffect(() => {
    ...
    fetch(`/api/fetch-all-user?id=${encodeURIComponent(userDetails.referenceid)}`)
      .then((r) => r.json()).then((d) => setAgents(d ?? [])).catch(() => setAgents([]));
  ...
  const agentMap = useMemo(() => {
    ...
    agents.forEach((a) => { if (a.ReferenceID) map[a.ReferenceID.toLowerCase()] = `${a.Firstname} ${a.Lastname}`; });
    return map;
  }, [agents]);
  ...
  const scopedAccounts = useMemo(() => {
    if (!hasAgentFilter) return allActiveAccounts;
    return allActiveAccounts.filter((a) => a.referenceid?.toLowerCase() === ownerFilter);
  }, [allActiveAccounts, ownerFilter, hasAgentFilter]);

  const scopedActivities = useMemo(() => {
    let acts = allActivities;
    if (hasAgentFilter) acts = acts.filter((a) => a.referenceid?.toLowerCase() === ownerFilter);
    if (hasDateFilter)  acts = acts.filter((a) => activityInRange(a.date_created, dateCreatedFilterRange));
    return acts;
  }, [allActivities, ownerFilter, hasAgentFilter, dateCreatedFilterRange, hasDateFilter]);

  const companiesWithActivity = useMemo(() => {
    const s = new Set<string>();
    scopedActivities.forEach((a) => { if (a.company_name) s.add(a.company_name.toLowerCase()); });
    return s;
  }, [scopedActivities]);
  ...
  const withActivityAccounts    = useMemo(() => scopedAccounts.filter((a) => companiesWithActivity.has(a.company_name.toLowerCase())), [scopedAccounts, companiesWithActivity]);
  const withoutActivityAccounts = useMemo(() => scopedAccounts.filter((a) => !companiesWithActivity.has(a.company_name.toLowerCase())), [scopedAccounts, companiesWithActivity]);

  const ownerOptions = useMemo(() => {
    const ids = [...new Set(allActiveAccounts.map((a) => a.referenceid?.toLowerCase()).filter(Boolean))];
    return ids.map((id) => ({ value: id, label: agentMap[id] ?? id }));
  }, [allActiveAccounts, agentMap]);

  const typeClientCounts = useMemo(() => {
    const c: Record<string, number> = {};
    scopedAccounts.forEach((a) => { const t = a.type_client ?? "Unknown"; c[t] = (c[t] ?? 0) + 1; });
  ...
  const openHistory = async (companyName: string, source: ListSource = null) => {
    ...
    try {
      ...
      setActivities(all.filter((a) =>
        ...
        (!hasAgentFilter || a.referenceid?.toLowerCase() === ownerFilter)
      ));
  ...
  return (
    <>
      <ExportDialog open={exportOpen} onClose={() => setExportOpen(false)} rows={filteredAccounts} agentMap={agentMap} />
      ...
      <AccountListDialog
        ...
        source="with"
        title="Accounts With Activities"
        description={listDesc(withActivityAccounts.length, "with")}
        icon={<CheckCircle2 size={16} className="text-emerald-500" />}
        iconBg="bg-emerald-50 border-emerald-100"
        accounts={withActivityAccounts} agentMap={agentMap}
      ...
      <AccountListDialog
        ...
        source="without"
        title="Accounts With No Activities"
        description={listDesc(withoutActivityAccounts.length, "without")}
        icon={<AlertCircle size={16} className="text-amber-500" />}
        iconBg="bg-amber-50 border-amber-100"
        accounts={withoutActivityAccounts} agentMap={agentMap}
        activityCountMap={activityCountMap}
        onViewHistory={(n, s) => { setActiveListOpen(null); openHistory(n, s); }}
      />
      ...
      <div className="space-y-5">
        {(hasAgentFilter || hasDateFilter) && (
          <div className="flex flex-wrap gap-2">
            {hasAgentFilter && (
              <div className="flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-2 text-[11px] text-indigo-700 font-medium">
                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 shrink-0" />
                Filtered by agent: <strong>{agentLabel}</strong>
                <span className="text-indigo-400 text-[10px]">· {scopedAccounts.length} accounts</span>
              </div>
            )}
            {hasDateFilter && (
              <div className="flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-2 text-[11px] text-indigo-700 font-medium">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          ...
          <StatCard label="No Activities"   value={withoutActivityAccounts.length} accent="#f59e0b" clickable sublabel={hasDateFilter ? "in range" : undefined} onClick={() => setActiveListOpen("without")} />
        ...
        <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b flex flex-wrap items-center gap-3 bg-gray-50/60">
            <h2 className="text-sm font-semibold text-gray-700 mr-auto">
              Company Accounts
              <span className="ml-2 rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-600 border border-indigo-100">{filteredAccounts.length}</span>
            </h2>
            <select value={ownerFilter} onChange={(e) => setOwnerFilter(e.target.value)}
              ...
              {ownerOptions.map((o) => <option className="capitalize" key={o.value} value={o.value}>{o.label}</option>)}
            </select>
            ...
            <Table>
              ...
              <TableBody>
                {paginatedAccounts.length === 0 ? (
                  <TableRow><TableCell colSpan={10} className="text-center py-14 text-sm text-gray-400">No accounts found</TableCell></TableRow>
                  ...
                  return (
                    <TableRow key={account.id} className="hover:bg-indigo-50/20 transition-colors group">
                      <TableCell className="text-xs text-gray-600 font-medium whitespace-nowrap">
                        {agentMap[account.referenceid?.toLowerCase()] ?? <span className="text-gray-300 italic text-[11px]">Unassigned</span>}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            title={hasAct ? (hasDateFilter ? `${actCnt} activities in range` : `${actCnt} activities`) : (hasDateFilter ? "No activity in range" : "No activities")}
                            className={`w-1.5 h-1.5 rounded-full shrink-0 ${hasAct ? "bg-emerald-500" : "bg-amber-400"}`}
                          />
                          {actCnt > 0 && (
                            <span className="text-[10px] text-slate-400 tabular-nums">{actCnt}</span>
                          )}
                        </div>
                      </TableCell>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t bg-gray-50/50 text-xs text-gray-500">
            <span>
              Showing <span className="font-semibold text-gray-700">{Math.min((currentPage - 1) * ITEMS_PER_PAGE + 1, filteredAccounts.length)}–{Math.min(currentPage * ITEMS_PER_PAGE, filteredAccounts.length)}</span> of <span className="font-semibold text-gray-700">{filteredAccounts.length}</span>
            </span>
            <div className="flex items-center gap-1">
              <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className="border rounded-lg px-2.5 py-1 hover:bg-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors shadow-sm">← Prev</button>
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                const page = Math.max(1, Math.min(currentPage - 2, totalPages - 4)) + i;
                return page <= totalPages ? (
                  <button key={page} onClick={() => setCurrentPage(page)} className={`rounded-lg px-2.5 py-1 transition-colors font-medium shadow-sm ${page === currentPage ? "bg-indigo-600 text-white" : "border hover:bg-white"}`}>{page}</button>
                ) : null;
              })}
              <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="border rounded-lg px-2.5 py-1 hover:bg-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors shadow-sm">Next →</button>
            </div>
          </div>
        </div>
      </div>
