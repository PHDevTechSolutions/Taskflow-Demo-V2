...
export const CCG: React.FC<{
  ...
  const fetchActivities = useCallback(() => {
    ...
    const url = new URL("/api/activity/tsm/calendar/fetch", window.location.origin);
  ...
  // Agent options: only agents that actually appear in current activities
  ...
  // Filtered activities (search + all dropdowns incl. agent)
  const filteredActivities = useMemo(() => {
    ...
    return sortedActivities.filter((item) => {
      ...
      if (
        filterAgent !== "all" &&
        item.referenceid?.toLowerCase() !== filterAgent.toLowerCase()
      )
        return false;
      if (s) {
  ...
  // Dot map: unfiltered (dots don't vanish on filter changes)
