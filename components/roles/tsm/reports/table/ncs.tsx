...
interface NCS { id: number; quotation_amount?: number; quotation_number?: string; remarks?: string; date_created: string; date_updated?: string; company_name?: string; contact_number?: string; contact_person: string; type_client: string; type_activity?: string; status: string; referenceid: string; }
...
export const NCSTable: React.FC<NCSProps> = ({ referenceid, dateCreatedFilterRange, userDetails }) => {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`ncs:${referenceid}`).on("postgres_changes", { event: "*", schema: "public", table: "history", filter: `tsm=eq.${referenceid}` }, ({ eventType, new: n, old: o }: any) => {
