...
interface CSR { id: number; quotation_amount?: number; ticket_reference_number?: string; remarks?: string; date_created: string; date_updated?: string; company_name?: string; contact_number?: string; contact_person: string; type_client: string; status: string; referenceid: string; }
...
interface CSRProps { referenceid: string; target_quota?: string; dateCreatedFilterRange: any; setDateCreatedFilterRangeAction: React.Dispatch<React.SetStateAction<any>>; userDetails: UserDetails; }
...
export const CSRTable: React.FC<CSRProps> = ({ referenceid, dateCreatedFilterRange, userDetails }) => {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`csr:${referenceid}`).on("postgres_changes", { event: "*", schema: "public", table: "history", filter: `tsm=eq.${referenceid}` }, ({ eventType, new: n, old: o }: any) => {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("CSR Endorsements");

      // Add headers
      worksheet.columns = [
        { header: "Agent", key: "agent", width: 20 },
        { header: "Date", key: "date", width: 15 },
        { header: "Ticket Ref No.", key: "ticketRef", width: 20 },
      ...
      filtered.forEach((item) => {
        ...
        worksheet.addRow({
          ...
          ticketRef: item.ticket_reference_number || "-",
            ...
            : (
              <div className="overflow-x-auto rounded-xl border border-gray-100">
                <Table>
                  ...
                  <TableFooter><TableRow className="bg-gray-50 text-xs font-semibold font-mono"><TableCell colSpan={3} className="text-gray-500">Total</TableCell><TableCell className="text-right text-gray-800">{fmtPHP(total)}</TableCell><TableCell colSpan={4} /></TableRow></TableFooter>
                </Table>
              </div>
            )}
