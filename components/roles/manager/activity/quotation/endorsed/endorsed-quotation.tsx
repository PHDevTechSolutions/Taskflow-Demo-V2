...
// FIX: Removed unused import "ButtonGroup" and "CheckCircle2Icon"
import { supabase } from "@/utils/supabase";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import TaskListEditDialog from "../dialog/edit";

// FIX: Added missing fields to match what TaskListEditDialog expects
interface Completed {
    id: number;
...
export const EndorsedQuotation: React.FC<ScheduledProps> = ({
    ...
    const fetchActivities = useCallback(async () => {
        ...
        try {
            ...
            // FIX: Was using TSM endpoint "/api/activity/tsm/quotation/fetch"
            // Corrected to manager endpoint
    ...
    // Fetch agents
    // FIX: Was using "/api/fetch-all-user" (TSM endpoint).
    // Corrected to "/api/fetch-all-user-manager" to match pending-quotation.tsx
    useEffect(() => {
    ...
    const statusColor = (status: string) => {
        switch (status) {
            case "Approved By Sales Head": return "bg-emerald-600 text-white";
            case "Pending": return "bg-amber-500 text-white";
            case "Decline": return "bg-red-500 text-white";
            case "Decline By Sales Head": return "bg-red-500 text-white";
                    ...
                    {filteredActivities.map((item) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`absolute left-0 top-0 bottom-0 w-1 ${item.tsm_approved_status === "Approved By Sales Head" ? "bg-emerald-500" : item.tsm_approved_status === "Pending" ? "bg-amber-500" : "bg-gray-300"}`} />
