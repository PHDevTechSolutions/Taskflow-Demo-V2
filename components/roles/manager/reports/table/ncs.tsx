...
interface NCS { id: number; quotation_amount?: number; quotation_number?: string; remarks?: string; date_created: string; date_updated?: string; company_name?: string; contact_number?: string; contact_person?: string; type_client: string; type_activity?: string; status: string; referenceid: string; start_date?: string; end_date?: string; }
...
interface NCSProps { referenceid: string; target_quota?: string; dateCreatedFilterRange: any; setDateCreatedFilterRangeAction: React.Dispatch<React.SetStateAction<any>>; userDetails: UserDetails; }
...
export const NCSTable: React.FC<NCSProps> = ({ referenceid, dateCreatedFilterRange, userDetails }) => {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`ncs:${referenceid}`).on("postgres_changes", { event: "*", schema: "public", table: "history", filter: `manager=eq.${referenceid}` }, ({ eventType, new: n, old: o }: any) => {
  ...
  const filtered = useMemo(() => {
    ...
    return activities
      .filter(i => ["csr client", "tsa client", "new client"].includes((i.type_client ?? "").toLowerCase()))
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select value={selectedAgent} onValueChange={v => { setSelectedAgent(v); setPage(1); }}>
          ...
          <SelectContent><SelectItem value="all">All Agents</SelectItem>{agents.map(a => <SelectItem key={a.ReferenceID} value={a.ReferenceID}>{a.Firstname} {a.Lastname}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      {filtered.length > 0 && (
      ...
      {loading ? <div className="flex items-center justify-center py-10 text-xs text-gray-400">Loading...</div>
        ...
        : filtered.length === 0 ? <div className="flex items-center justify-center py-10 text-xs text-gray-400 italic">No new client records found.</div>
        : (
