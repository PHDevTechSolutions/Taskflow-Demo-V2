...
interface FB {
  ...
  contact_person?: string; source?: string; status: string;
}

export const FBTable: React.FC<{ referenceid: string; target_quota?: string; dateCreatedFilterRange: any; setDateCreatedFilterRangeAction: React.Dispatch<React.SetStateAction<any>>; }> = ({
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("FB Marketplace Report");
      ...
      let filename = "FB_Marketplace_Report";
      ...
      {filtered.length > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-indigo-50/50 border border-indigo-100/50">
          ...
          <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-wider">
            Unique quotations: <span className="text-indigo-900 font-mono underline underline-offset-2">{uniqueCount}</span>
          </p>
        </div>
      )}

      <TableShell loading={loading} error={error} empty={filtered.length === 0} emptyIcon="📘" emptyText="No Facebook Marketplace records found">
        <Table>
          <TableHeader>
