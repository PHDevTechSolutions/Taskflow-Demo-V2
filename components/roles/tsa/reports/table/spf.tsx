...
export const SPFTable: React.FC<{ referenceid: string; target_quota?: string; dateCreatedFilterRange: any; setDateCreatedFilterRangeAction: React.Dispatch<React.SetStateAction<any>>; }> = ({
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet("SPF Report");
          ...
          <TableBody>
            {paginated.map((item) => (
              <TableRow key={item.id} className="text-xs hover:bg-zinc-50/50 transition-colors border-b border-zinc-100 last:border-0">
                ...
                <TableCell className="text-right text-zinc-900 px-3 font-bold">{item.so_amount != null ? fmt(item.so_amount) : "—"}</TableCell>
                ...
                <TableCell className="capitalize text-zinc-500 px-3 font-medium">{item.call_type || "—"}</TableCell>
              </TableRow>
            ))}
          </TableBody>
          <tfoot>
