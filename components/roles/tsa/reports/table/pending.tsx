...
export const PendingTable: React.FC<{ referenceid: string; target_quota?: string; dateCreatedFilterRange: any; setDateCreatedFilterRangeAction: React.Dispatch<React.SetStateAction<any>>; }> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Info banner */}
      <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] text-amber-700 font-medium">
        ...
        <Table>
            ...
            {paginated.map((item) => {
              ...
              return (
                <TableRow key={item.id} className="text-xs hover:bg-slate-50/60 font-mono">
                  ...
                  <TableCell>
                    <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold ${daysPending >= 30 ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-700"}`}>
                      {daysPending}d
                    </span>
                  </TableCell>
          ...
          <tfoot>
            <TableRow className="bg-slate-50 font-semibold text-xs border-t border-slate-200">
              <TableCell colSpan={3} className="text-slate-500">Total ({filtered.length})</TableCell>
